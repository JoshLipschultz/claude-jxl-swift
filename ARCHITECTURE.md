...
| M10 | macOS integration | `CGImage` bridge + Quick Look thumbnail/preview appex | ✅ **Quick Look thumbnails work** — `JXLQuickLook.appex` (decode via JXLCore, profile-aware `CGImage` via JXLKit) embedded in JXLViewer.app, built with Xcode-beta via xcodegen; verified with `qlmanage` on real camera files. `swift build`/`swift test` also work under `DEVELOPER_DIR=/Applications/Xcode-beta.app/...`. Remaining: a dedicated full-size preview extension (spacebar previews currently scale the thumbnail) |
